...
- **`plain`**: one line per event, in order, via `typer.echo` — today's output, unchanged in spirit. The one rule that isn't "print `text`": a `heartbeat` prints only when `state` differs from the last `heartbeat` shown for that `issue_id`, or when `PLAIN_KEEPALIVE_MS` (`renderer.py`, 60s) has passed since the last one shown for it, whichever comes first. That turns the 360 identical polls in the epic's example into one line per state transition (`dispatched` → `working` → `verifying` → `reaping`), plus an occasional keepalive so a long turn stuck in one state does not look hung in a piped log, without suppressing heartbeats a renderer without a table still needs. The cadence is `PLAIN_KEEPALIVE_MS`, not `[run] poll_ms`: how often milhouse asks herdr and how often a human wants to be told are unrelated questions, and coupling them is what produced the wall of text in the first place.
