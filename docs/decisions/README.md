...
| [0026](0026-the-progress-channel-is-events-and-the-terminal-is-one-renderer.md) | The progress channel is events, and the terminal is one renderer     |
...
- **The per-lane bootstrap, and a gate that fails for the wrong reason.** A fresh worktree has no `.venv`, and `[verify] command` now runs in the lane rather than the primary checkout. [ADR 0023](0023-a-run-has-one-lane.md) paid that once per run; [ADR 0024](0024-an-integration-lane-and-worker-lanes.md) gives it back once per issue, so a `--count 4` run bootstraps four worktrees at once.

  The cost half is now measured, against this repository, and it is small. A fresh worktree running `uv run pytest` takes about two seconds and produces an 84M `.venv` of which roughly three quarters is hardlinked from the uv cache. A cache with nothing in it at all costs the same two seconds. `uv run` bootstraps on its own, so nothing fails and no setup command is needed. Two qualifications: the sharing is a filesystem property rather than a uv one, and a worktree on a different filesystem from the cache falls back to a full 84M copy; and uv is close to the best case, since a gate built on `npm ci` gets an unshared `node_modules` per lane and one on `cargo test` a cold `target/`, which is minutes and gigabytes rather than seconds.

  What is actually unresolved is not the cost. **A gate that fails because the lane was never set up is indistinguishable from a gate that fails because the code is broken**, and this epic made milhouse act on that ambiguity more loudly rather than less. A red gate in a worker lane spends one of the issue's attempts, and [ADR 0024](0024-an-integration-lane-and-worker-lanes.md)'s integration gate halts the run and writes the failing output as a note onto an issue that is already closed. On a project whose gate does not bootstrap itself, an environmental failure therefore stops a run and leaves a misleading note on work that was fine. Lanes needing a setup command is one answer; letting a gate report "could not run" separately from "ran and failed" is another, and probably the better one.
