...
- **Landing the lanes, for `dispatch`.** N lanes that each verify green can be red combined, which serial work on one branch could never produce. [ADR 0024](0024-an-integration-lane-and-worker-lanes.md) answers it for `milhouse run`: worker branches are merged into the integration branch one at a time, and a merge that joined anything re-runs the gate on the result. `dispatch` still merges nothing, and `bd merge-slot` is still a coordination primitive that does not perform the merge. Whether the run's answer generalises to lanes another process opened is unanswered, and the first concurrent run watched end to end is what would say.
- **When to reap.** `milhouse reap` collects whatever has settled and is safe to run at any time, so `until milhouse reap; do sleep 60; done` works. [ADR 0022](0022-the-loop-is-earned.md) answers this for the serial case, where the loop waits on each turn, and [ADR 0024](0024-an-integration-lane-and-worker-lanes.md) answers it for a concurrent run, which polls its own lanes at `[run] poll_ms`. What stays open is the mechanism for lanes milhouse did not start: the [ADR 0001](0001-shell-out-to-bd-and-herdr.md) revisit and the socket API's `events.subscribe` is still the better one, and polling a run's own lanes was not enough to earn it.
- **The per-lane bootstrap.** A fresh worktree has no `.venv`, and `[verify] command` now runs in the lane rather than the primary checkout, so a gate that assumes one fails for environmental reasons rather than real ones. Lanes need a setup command, and its shape is unknown. This got worse rather than better: [ADR 0023](0023-a-run-has-one-lane.md) paid the tax once per run, and [ADR 0024](0024-an-integration-lane-and-worker-lanes.md) gives that back, once per issue and several at a time.
