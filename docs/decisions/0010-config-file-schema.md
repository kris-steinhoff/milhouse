...
The config file was referenced by the design and never defined. It needs to hold at least the agent kind and its args, the run budget, and the turn timeout, without becoming a place where every internal constant leaks into a user-facing contract.
...
| `[loop]`    | `max_iterations`, `turn_timeout_ms`             |
...
Adding a key means adding a field to the pydantic model _and_ a row to `docs/configuration.md`. That coupling is deliberate: an undocumented config key is a bug. `[loop]` shrank when the unattended policy surface was de-scoped ([ADR 0014](0014-step-is-the-primitive.md)); removing a key is the same coupling in reverse.
