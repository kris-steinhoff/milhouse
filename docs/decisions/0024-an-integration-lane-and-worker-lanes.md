...
### A merge that did not land is the last merge of the run

The first merge that does not land closes the integration branch for the rest of the run. Every turn after it is still reaped, still verified in its own lane, still recorded and still settled, and its branch is left exactly where it is, with a record naming the branch that has to be landed before it.

The reason is the merge order rather than the cost. What "merges are serialized" buys is that the integration branch is always a prefix of one known order, which is what makes a failure reproducible from the report. A merge that does not land ends that: the branch is now the prefix up to the failure, and the run has an unlanded branch it has told somebody to land by hand. Landing a later branch on top would move the target out from under that instruction. The resolution the report described is no longer the resolution they would be doing, and the branch they were told to land first is no longer the first thing missing. Two branches to land in a stated order is a mess. The same two landed in the wrong order, onto a branch that moved in between, is a worse one.

The cost argument is real and secondary. A merge into a branch that has just refused one is close to certain to fail the same way when the turns touched the same lines, which in every watched run they did. It is not the expensive part, though: the worker lane's gate runs to decide whether the turn succeeded at all, not to decide whether to merge it, so it is paid either way. What is saved is the integration gate, and only for a merge that would have joined something.

**This does not change what `conflict` means as a halt reason.** The row is unchanged: a merge that did not land stops the run, the issue stays closed, both branches are named, and a person lands it. What changes is what the drain does with the turns already in flight, and `MergeRecord` gains a third way of not having landed (nobody asked git) alongside a conflict and a refusal. All three are the same consequence, so the halt table keeps one row and `merge_line` says which happened.

The rule lives on the session rather than in the drain, because the drain is not the only place it fires. Turns that settle in the same reap pass are all merged before `run()` is handed the first of them, so in the watched run one further merge had already happened by the time there was a halt to drain.

A red integration branch is different, and does not close it. The merge landed, the branch still contains every predecessor in order, and a later merge onto it stays a prefix of the same order. It is a halt for what is on the branch, not for what is missing from it.
...
**Keep merging after a merge has failed, on the argument that a later turn may touch nothing the failed one did.** Rejected above, and the argument is true: a run wide enough to work unrelated files will sometimes have a mergeable branch behind an unmergeable one. It is rejected because landing it is not the favour it looks like. The person still has to land the failed branch by hand, and doing that against an integration branch that has since taken a later merge is a different and harder resolution than the one the report described. Four watched merges failed and none of the branches behind them was independent of the failure, so the case being given up is one that has not yet been observed, and giving it up costs one merge somebody does by hand.

**Abandon the turns in flight once a merge has failed, since none of them can land.** Rejected, and not seriously considered. A turn that is running has a claimed issue and a live agent, and dropping it is the thing the drain exists to prevent. The work is paid for either way, its commits are on its branch either way, and the issue is closed by it either way. Only the merge is skipped.

**Merge but skip the integration gate once the branch is known bad.** Rejected. It keeps the ordering problem, which is the actual objection, and saves the cheaper half. The gate is skipped by this decision anyway, because a merge nobody attempted joins nothing.
...
- **The per-lane bootstrap tax is per issue again, multiplied by the count.** ADR 0023 counted paying it once per run as a real improvement, and this gives that back. A fresh worktree still has no `.venv`, `[verify] command` still runs in the lane, and a `--count 4` run now pays for four of them at once. Measured against this repository afterwards, the tax itself is small: about two seconds and a mostly-hardlinked `.venv` per lane, because `uv run` bootstraps on its own. A toolchain that does not, such as `npm ci` or `cargo test`, pays minutes and gigabytes per lane instead. What the count multiplies is therefore the risk rather than the cost — a gate that cannot run in a fresh lane now fails in four of them, and milhouse cannot tell that from four real failures. Still [open](README.md#still-open) for that reason, not for the seconds.
...
- **A conflict in a wide run leaves more than one branch, and the order matters.** Every turn still in flight finishes and none of them is merged, so a `--count 4` run can end with one branch on the integration branch and three beside it. The report lists them in merge order and says the first one stopped the run, because landing them in a different order is a different set of conflicts.
- **A branch that would have merged cleanly behind a failure is not merged either.** milhouse does not find out whether it would have, and there is one hand-landing to do that a run which kept trying would have avoided. The alternative was changing the resolution somebody had already been told to do.
...
**The drain finished what it started, and made the mess bigger.** A conflict halt fired in a `--count 4` run with one turn still working. The run printed `draining 1 turn(s) already in flight; they are not abandoned, and a successful one is still merged`, waited five poll intervals, reaped the turn, verified it, merged it — and that merge conflicted too, into the same integration branch that had already refused two. The run ended `4 issue(s) closed, 1 merged` with three closed issues on three live branches. Nothing here contradicts the design: `RunResult.unmerged` already says a drain can produce more than one, and it did. But merging into a branch that has just refused a merge is close to certain to fail again, and each failure costs a full gate run in the worker lane first. That was filed as `milhouse-amd.13` rather than fixed here, because it is a change to what a halt means, and it is now decided above: a merge that did not land is the last merge of the run. The same run today ends with the same four closed issues and the same one merge, and says `4 issue(s) closed, 1 merged, 3 not merged`, with the three branches listed in the order they have to be landed. Two things it did are gone: the merge attempted into a branch that had already refused one, and the line promising that a drained success is still merged.
...
**A turn `dispatch` could not start is handed back, and it ends the dispatch.** `dispatch` used to settle such a turn and carry on to the next issue, discarding the `StepResult`. Two things followed from that, and neither is `milhouse-amd.12`. The iteration is recorded as `error`, which is a row of the halt table, but `Parallel` never learned the turn existed — so a run whose agent side was sick ended `nothing is ready but N issue(s) are unfinished`, which describes a deadlocked dependency graph and not what happened. And `_finish` re-opens the issue it settles, so `bd ready` offered the same issue straight back to the next pass of the same loop: at `--count 3`, one issue whose agent will not take a prompt could be charged three attempts and deferred inside a single call, which is the retry ladder spent at poll speed again, arriving by a different route.

So `dispatch` returns a `DispatchResult` — what it started, and the turn it could not — and it stops at the first failure. Stopping needs no judgement about which failure it was, because the two kinds surface in different places. A failure to prepare **one issue**, such as a lane branch that is already taken, is raised by `step._prepare` and never reaches the branch that settles a turn. Everything that does reach it is the **agent side**: an agent that would not start in a pane, or a prompt herdr could not get taken. The next issue would be handed to the same herdr and the same agent binary, so carrying on charges another issue an attempt for something that is wrong somewhere else. Should a preparation failure ever be settled there rather than raised, it belongs in a third list that does not end the call.

The halt table is unchanged by this. The `error` row already said what to do with a turn that milhouse itself could not run, and the fix is that the turn now gets there. Filed and fixed as `milhouse-amd.14`.
...
The integration gate is observed either catching something real or never catching anything, since the first justifies its doubled cost and the second retires it. Or when a gate can say "could not run here" separately from "ran and failed", since that is what stops a lane that was never set up from spending an issue's attempts and halting a run, and it is the half of the per-lane bootstrap that measuring the tax did not answer.
